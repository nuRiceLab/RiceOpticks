...
generate_torch_photon(Photon& p, TorchStep& ts, curandState &rng)
{
      ...
      if(ts.mode & M_WAVELENGTH_COMB )   
      { 
          p.wavelength = comb_lookup(curand_uniform(&rng)) ;
      }
      else if( ts.mode & M_WAVELENGTH_SOURCE )
      {
          p.wavelength = source_lookup(curand_uniform(&rng)) ;
      }
      else
      {
